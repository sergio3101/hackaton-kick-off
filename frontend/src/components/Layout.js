...
export default function Layout() {
    ...
    const crumbs = useMemo(() => {
        ...
        if (path === "/")
            return ["Рабочая область", "Дашборд"];
        if (path === "/projects")
            return ["Проекты"];
        if (path === "/upload")
            return ["Загрузить ТЗ"];
